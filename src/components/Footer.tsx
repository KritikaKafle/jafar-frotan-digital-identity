        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 text-center md:text-left">
          ...
          <div className="space-y-3 md:space-y-4">
            ...
            <nav className="space-y-1 md:space-y-2">
              {quickLinks.map(link => <a key={link.href} href={link.href} className={`block text-muted-foreground transition-colors text-xs md:text-sm ${link.label === 'Zeytoon ICT' ? 'hover:text-[#808000]' : 'hover:text-primary'}`}>
                  {link.label}
                </a>)}
            </nav>
          </div>
          ...
          {/* Projects */}
          <div className="space-y-3 md:space-y-4">
            <h4 className="font-semibold text-foreground text-sm md:text-base">Projects</h4>
            <nav className="space-y-1 md:space-y-2">
              {externalLinks.map(link => <a key={link.href} href={link.href} className={`block text-muted-foreground transition-colors text-xs md:text-sm ${link.label === 'Zeytoon ICT' ? 'hover:text-[#808000]' : link.label === 'Hekayat Shab' ? 'hover:text-[#f6c916]' : 'hover:text-primary'}`} target={link.href.startsWith('http') ? '_blank' : undefined} rel={link.href.startsWith('http') ? 'noopener noreferrer' : undefined}>
                  {link.label}
                </a>)}
            </nav>
          </div>
          ...
          <div className="col-span-2 md:col-span-1 space-y-3 md:space-y-4">
            <h4 className="font-semibold text-foreground text-sm md:text-base">Connect</h4>
